...
public final class SyndicateParticipantFlow {
    private SyndicateParticipantFlow(){
    ...
    public static class Initiator extends FlowLogic<SignedTransaction> {
        ...
        public SignedTransaction call() throws FlowException {
            ...
            Party bank = getOurIdentity();

            StateAndRef<LoanState> loan = getLoanById(loanId);
            LoanState loanState = loan.getState().getData();

            SyndicateParticipantState syndicateParticipantState = new SyndicateParticipantState(
                    new UniqueIdentifier(),
                    new StaticPointer<>(loan.getRef(), LoanState.class, true),
                    bank,
                    issuedLoanSum,
                    loanState.getBorrower(),
                    loanState.getAgentBank()
            );
            ...
            builder.addOutputState(syndicateParticipantState);
            builder.addCommand(
                    new SyndicateParticipantContract.Commands.Submit(),
                    syndicateParticipantState.getParticipants().stream()
            ...
            List<FlowSession> cpSessions = syndicateParticipantState.getParticipants().stream()
                    .filter(party -> !Objects.equals(party, bank))
